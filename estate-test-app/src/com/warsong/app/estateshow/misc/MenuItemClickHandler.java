...
public class MenuItemClickHandler {
	...
	public void exec() {
		if (menuItem != null) {
			...
			if (JumpHelper.KEY_ONE.equals(key)) { //到首页
				JumpHelper.jump(context, key, b);
			} else if (JumpHelper.KEY_NEWS.equals(key)) { //到新闻
				String url = menuItem.getUrl();
				if (!StringUtil.isEmpty(url)) {
					b.putString("url", url);
					JumpHelper.jump(context, key, b);
				}
			} else if (JumpHelper.KEY_MAP.equals(key)) {//到地图
				b.putString("title", menuItem.getTitle());
				...
				JumpHelper.jump(context, key, b);
			} else if (JumpHelper.KEY_PHOTOS.equals(key)) { //到图片浏览
				b.putString("title", menuItem.getTitle());
				...
				JumpHelper.jump(context, key, b);
			} else if (JumpHelper.KEY_HTMLVIEW.equals(key)) { //到html view
				String url = menuItem.getUrl();
				if (!StringUtil.isEmpty(url)) {
					b.putString("url", url);
					JumpHelper.jump(context, key, b);
				}
			} else if (JumpHelper.KEY_SETTING.equals(key)) {
